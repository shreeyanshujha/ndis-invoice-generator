...
export default function Invoice() {
  ...
  const { user, loading, isPaid, logOut } = useAuth();
  ...
  const generatePDF = () => {
    if (!isPaid && invoiceCount >= 3) {
      const confirmUpgrade = window.confirm(
        'You’ve hit the free invoice limit. Upgrade now to unlock unlimited access.'
      );
      if (confirmUpgrade) {
        window.location.href = 'https://buy.stripe.com/dR6cMR1j63y0fFm3cc';
      }
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center items-start p-6">
      <div className="bg-white w-full max-w-4xl rounded-xl shadow-md p-8 relative">
